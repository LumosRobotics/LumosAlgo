#ifndef DUOPLOT_MATH_IMAGE_IMAGE_RGB_H_
#define DUOPLOT_MATH_IMAGE_IMAGE_RGB_H_
...
namespace lumos
{
    ...
    {
        DUOPLOT_ASSERT(num_rows > 0U) << "Cannot initialize with number of rows to 0!";
        DUOPLOT_ASSERT(num_cols > 0U) << "Cannot initialize with number of columns to 0!";
    ...
    {
        DUOPLOT_ASSERT(other.numRows() > 0U) << "Cannot initialize with number of rows to 0!";
        DUOPLOT_ASSERT(other.numCols() > 0U) << "Cannot initialize with number of columns to 0!";
    ...
    template <typename T>
    size_t ImageRGB<T>::numRows() const
    {
        return num_rows_;
    }

    template <typename T>
    size_t ImageRGB<T>::numCols() const
    {
        return num_cols_;
    }

    template <typename T>
    T *ImageRGB<T>::data() const
    {
        return data_;
    }
    ...
    template <typename T>
    const T &ImageRGB<T>::operator()(const size_t r, const size_t c, const size_t ch) const
    {
        assert((r < num_rows_) && "Row index is larger than num_rows_ - 1!");
        assert((c < num_cols_) && "Column index is larger than num_cols_ - 1!");
        assert((ch < 3) && "Channel index is larger than 2!");
...
#endif // DUOPLOT_MATH_IMAGE_IMAGE_RGB_H_
