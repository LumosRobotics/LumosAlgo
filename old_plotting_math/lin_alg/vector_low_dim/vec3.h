#ifndef DUOPLOT_MATH_LIN_ALG_VECTOR_LOW_DIM_VEC3_H_
#define DUOPLOT_MATH_LIN_ALG_VECTOR_LOW_DIM_VEC3_H_

#include <cmath>

#include "lumos/plotting/math/lin_alg/vector_low_dim/class_def/vec3.h"
...
namespace lumos
{
    ...
    template <typename T>
    Vec3<T> Vec3<T>::vectorBetweenPoints(const Point3<T> &end_point) const
    {
        // "This" is start point
    ...
    template <typename T>
    Vec3<T> Vec3<T>::normalizedVectorBetweenPoints(const Point3<T> &end_point) const
    {
        // "This" is start point
    ...
    template <typename T>
    Vec3<T> Vec3<T>::elementWiseMultiply(const Vec3<T> &factor_vector) const
    {
        return Vec3<T>(x * factor_vector.x, y * factor_vector.y, z * factor_vector.z);
    }

    template <typename T>
    Vec3<T> Vec3<T>::elementWiseDivide(const Vec3<T> &denominator_vector) const
    {
        return Vec3<T>(x / denominator_vector.x, y / denominator_vector.y, z / denominator_vector.z);
    }

    template <typename T>
    Vec3<T> Vec3<T>::crossProduct(const Vec3<T> &right_vector) const
    {
    ...
    template <typename T>
    Vec3<T> operator*(const Vec3<T> &v, const T f)
    {
        return Vec3<T>(f * v.x, f * v.y, f * v.z);
    }

    template <typename T>
    T operator*(const Vec3<T> &v0, const Vec3<T> &v1)
    {
        return v0.x * v1.x + v0.y * v1.y + v0.z * v1.z;
    }
    ...
    template <typename T>
    Vec3<T> operator*(const Matrix<T> &m, const Vec3<T> &v)
    {
        DUOPLOT_ASSERT(m.numRows() == 3) << "Matrix dimension mismatch!";
        DUOPLOT_ASSERT(m.numCols() == 3) << "Matrix dimension mismatch!";
        Vec3<T> res;
    ...
    template <typename T>
    Vec3<T> operator*(const MatrixFixed<T, 3, 3> &m, const Vec3<T> &v)
    {
        Vec3<T> res;
        res.x = m(0, 0) * v.x + m(0, 1) * v.y + m(0, 2) * v.z;
        res.y = m(1, 0) * v.x + m(1, 1) * v.y + m(1, 2) * v.z;
...
#endif // DUOPLOT_MATH_LIN_ALG_VECTOR_LOW_DIM_VEC3_H_
