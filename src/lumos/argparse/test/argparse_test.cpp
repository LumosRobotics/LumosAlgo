...
namespace lumos
{
    namespace argparse
    {
        ...
        // Test argument not found
        TEST_F(ArgparseTest, ArgumentNotFound)
        {
            parser->addArgument("--existing", Argument::Type::STRING);
            parser->parseArgs(std::vector<std::string>{});

            EXPECT_THROW(parser->get<std::string>("nonexistent"), ArgumentError);
            EXPECT_FALSE(parser->isSet("nonexistent"));
        }

        // Test type mismatch
        TEST_F(ArgparseTest, TypeMismatch)
        {
            parser->addArgument("--count", Argument::Type::INT).defaultValue("42");
            parser->parseArgs(std::vector<std::string>{});

            // Try to get int as string type (should throw)
            EXPECT_THROW(parser->get<std::string>("count"), ArgumentTypeError);
        }
        ...
        // Test edge cases
        TEST_F(ArgparseTest, EdgeCases)
        {
            parser->addArgument("--empty", Argument::Type::STRING);

            // Empty string value
            std::vector<std::string> args = {"--empty", ""};
            parser->parseArgs(args);
            EXPECT_EQ(parser->get<std::string>("empty"), "");
            EXPECT_TRUE(parser->isSet("empty"));
        }

        // Test complex scenario
        TEST_F(ArgparseTest, ComplexScenario)
        {
            parser->addArgument("input_file").help("Input file");
            parser->addArgument("--output", Argument::Type::STRING).defaultValue("output.txt").help("Output file");
            ...
            parser->addArgument("--include", Argument::Type::STRING_LIST).nargs("*").help("Files to include");
            ...
            std::vector<std::string> args = {
                ...
                "--threads", "8",
                "--format", "xml",
                "--include", "header.h", "utils.cpp"};

            parser->parseArgs(args);

            EXPECT_EQ(parser->get<std::string>("input_file"), "input.txt");
            EXPECT_EQ(parser->get<std::string>("output"), "output.txt"); // Default
            EXPECT_TRUE(parser->get<bool>("verbose"));
            EXPECT_EQ(parser->get<int>("threads"), 8);
            EXPECT_EQ(parser->get<std::string>("format"), "xml");

            auto includes = parser->get<std::vector<std::string>>("include");
            EXPECT_EQ(includes.size(), 2);
            EXPECT_EQ(includes[0], "header.h");
            EXPECT_EQ(includes[1], "utils.cpp");
        }
