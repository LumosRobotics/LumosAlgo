...
namespace lumos
{
  ...
  template <typename T>
  size_t Vector<T>::numElements() const { return size_; }
  ...
  template <typename T>
  T *Vector<T>::data() const { return data_; }

  template <typename T>
  T *Vector<T>::begin() const { return &data_[0]; }

  template <typename T>
  T *Vector<T>::end() const { return &data_[size_]; }
