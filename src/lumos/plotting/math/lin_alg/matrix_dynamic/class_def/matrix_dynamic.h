#ifndef DUOPLOT_MATH_LIN_ALG_MATRIX_DYNAMIC_CLASS_DEF_MATRIX_DYNAMIC_H_
#define DUOPLOT_MATH_LIN_ALG_MATRIX_DYNAMIC_CLASS_DEF_MATRIX_DYNAMIC_H_
...
namespace duoplot
{
    ...
    public:
        ...
        {
            DUOPLOT_ASSERT(il.size() > 0) << "Tried to initialize with empty vector matrix!";
            DUOPLOT_ASSERT(il.begin()[0].size() > 0) << "Tried to initialize with empty vector matrix!";

            for (size_t r = 0; r < il.size(); r++)
            {
                DUOPLOT_ASSERT(il.begin()[0].size() == il.begin()[r].size())
