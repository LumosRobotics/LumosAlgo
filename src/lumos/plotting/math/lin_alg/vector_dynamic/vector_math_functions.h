...
namespace duoplot
{
    ...
    template <typename T>
    Vector<T> abs(const Vector<T> &vin)
    {
        DUOPLOT_ASSERT(vin.size() > 0);
    ...
    template <typename T>
    Vector<T> tan(const Vector<T> &vin)
    {
        DUOPLOT_ASSERT(vin.size() > 0);
        Vector<T> v(vin.size());
        for (size_t k = 0; k < vin.size(); k++)
    ...
    template <typename T>
    Vector<T> atan(const Vector<T> &vin)
    {
        DUOPLOT_ASSERT(vin.size() > 0);
        Vector<T> v(vin.size());
        for (size_t k = 0; k < vin.size(); k++)
    ...
    template <typename T>
    Vector<T> linspaceFromStartPointIncAndCount(const T x0, const T dx, const size_t num_values)
    {
        DUOPLOT_ASSERT(num_values > 0);
        Vector<T> v(num_values);
