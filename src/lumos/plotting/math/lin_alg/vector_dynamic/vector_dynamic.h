#ifndef DUOPLOT_MATH_LIN_ALG_VECTOR_DYNAMIC_VECTOR_DYNAMIC_H_
#define DUOPLOT_MATH_LIN_ALG_VECTOR_DYNAMIC_VECTOR_DYNAMIC_H_
...
namespace duoplot
{
    ...
    template <typename T>
    Vector<T>::Vector(const size_t vector_length)
    {
        DUOPLOT_ASSERT(vector_length > 0U) << "Length of vector can't be 0!";
        size_ = vector_length;
    ...
    {
        DUOPLOT_ASSERT(v.size() > 0U) << "Input vector size is 0!";
        data_ = v.data();
        size_ = v.size();
    ...
    template <typename T>
    size_t Vector<T>::numElements() const
    {
        return size_;
    }

    template <typename T>
    size_t Vector<T>::numBytes() const
    {
    ...
    {
        DUOPLOT_ASSERT(size_ > 0U) << "Tried to fill unallocated vector!";
    ...
    {
        DUOPLOT_ASSERT(new_size > 0U) << "Length of vector can't be 0!";
        if (new_size != size_)
        {
            if (size_ > 0U)
            {
    ...
    {
        DUOPLOT_ASSERT(v0.size() == v1.size());
        T d = 0.0;
    ...
    template <typename T>
    Vector<T> operator%(const Vector<T> &v, const int i)
    {
