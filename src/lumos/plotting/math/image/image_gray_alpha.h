#ifndef DUOPLOT_MATH_IMAGE_IMAGE_GRAY_ALPHA_H_
#define DUOPLOT_MATH_IMAGE_IMAGE_GRAY_ALPHA_H_
...
namespace lumos
{
    ...
    template <typename T>
    ImageGrayAlpha<T>::ImageGrayAlpha(const size_t num_rows, const size_t num_cols)
    {
    ...
    template <typename T>
    T *ImageGrayAlpha<T>::data() const
    {
        return data_;
    }

    template <typename T>
    T &ImageGrayAlpha<T>::operator()(const size_t r, const size_t c, const size_t ch)
    {
        assert((r < num_rows_) && "Row index is larger than num_rows_ - 1!");
        assert((c < num_cols_) && "Column index is larger than num_cols_ - 1!");
        assert((ch < 2) && "Channel index is larger than 1!");
    ...
    template <typename T>
    const T &ImageGrayAlpha<T>::operator()(const size_t r, const size_t c, const size_t ch) const
    {
        assert((r < num_rows_) && "Row index is larger than num_rows_ - 1!");
        assert((c < num_cols_) && "Column index is larger than num_cols_ - 1!");
        assert((ch < 2) && "Channel index is larger than 1!");
...
#endif // DUOPLOT_MATH_IMAGE_IMAGE_GRAY_ALPHA_H_
