#ifndef DUOPLOT_MATH_IMAGE_IMAGE_RGBA_H_
#define DUOPLOT_MATH_IMAGE_IMAGE_RGBA_H_
...
namespace duoplot
{
    ...
    template <typename T>
    ImageRGBA<T>::ImageRGBA(const ImageRGBA<T> &other_image)
    {
    ...
    template <typename T>
    size_t ImageRGBA<T>::numBytes() const
    {
        return 4 * num_rows_ * num_cols_ * sizeof(T);
    }

    template <typename T>
    size_t ImageRGBA<T>::numElements() const
    {
        return 4 * num_rows_ * num_cols_;
    }

    template <typename T>
    size_t ImageRGBA<T>::numRows() const
    {
        return num_rows_;
    }

    template <typename T>
    size_t ImageRGBA<T>::numCols() const
    {
        return num_cols_;
    }

    template <typename T>
    T *ImageRGBA<T>::data() const
    {
        return data_;
    }
    ...
    template <typename T>
    const T &ImageRGBA<T>::operator()(const size_t r, const size_t c, const size_t ch) const
    {
        assert((r < num_rows_) && "Row index is larger than num_rows_ - 1!");
        assert((c < num_cols_) && "Column index is larger than num_cols_ - 1!");
        assert((ch < 4) && "Channel index is larger than 3!");
    ...
    template <typename T>
    void ImageRGBA<T>::remapChannels(const std::array<std::uint8_t, 3U> &map_values)
    {
