...
inline std::string getWhiteColorString() { return "\033[0m"; }
...
inline std::string getPreString(const MessageSeverity msg_severity,
  ...
  const std::string severity_color =
      getUseColors() ? getSeverityColor(msg_severity) : "";
  ...
  const std::string func_string =
      getShowFunc() ? "[ " + std::string(func_name) + " ]" : "";

  const std::string file_string =
      getShowFile() ? "[ " + file_name_no_path + " ]" : "";
  const std::string line_number_string =
      getShowLineNumber() ? "[ " + std::to_string(line_number) + " ]" : "";
  const std::string severity_string =
      "[ " + getSeverityString(msg_severity) + " ]";
  const std::string thread_id_string =
      getShowThreadId() ? "[ 0x" + decNumberAsHexString(getThreadId()) + " ]"
                        : "";

  return severity_color + severity_string + thread_id_string + file_string +
...
#define PRINT() lumos::logging::internal::Log().getStream()

#define PRINT_COND(cond) lumos::logging::internal::Log(cond).getStream()
