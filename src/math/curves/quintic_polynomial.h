...
#include "math/curves/class_def/quintic_polynomial.h"
#include <algorithm>
#include <cmath>
#include <stdexcept>
...
template <typename T> T QuinticPolynomial1D<T>::evaluate(T t) const {
  t = std::max(T(0), std::min(duration_, t));
...
template <typename T> T QuinticPolynomial1D<T>::evaluateVelocity(T t) const {
  t = std::max(T(0), std::min(duration_, t));
...
template <typename T> T QuinticPolynomial1D<T>::evaluateJerk(T t) const {
  t = std::max(T(0), std::min(duration_, t));
...
template <typename T> T QuinticPolynomial1D<T>::evaluateSnap(T t) const {
  t = std::max(T(0), std::min(duration_, t));

  return T(24) * coefficients_[4] + T(120) * coefficients_[5] * t;
}

template <typename T> T QuinticPolynomial1D<T>::evaluateCrackle(T) const {
  return T(120) * coefficients_[5];
}

template <typename T> T QuinticPolynomial1D<T>::getDuration() const {
  return duration_;
}
