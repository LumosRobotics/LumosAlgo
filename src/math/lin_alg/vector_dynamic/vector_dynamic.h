#ifndef LUMOS_MATH_LIN_ALG_VECTOR_DYNAMIC_VECTOR_DYNAMIC_H_
#define LUMOS_MATH_LIN_ALG_VECTOR_DYNAMIC_VECTOR_DYNAMIC_H_
...
#include "logging.h"
#include "math/lin_alg/vector_dynamic/class_def/vector_dynamic.h"
...
template <typename T> Vector<T>::Vector(Vector<T> &&v) {
  ASSERT(v.size() > 0U) << "Input vector size is 0!";
  data_ = v.data();
  size_ = v.size();
...
template <typename T>
template <typename Y>
Vector<T>::Vector(const Vector<Y> &v) {
  size_ = v.size();
...
template <typename T> size_t Vector<T>::numElements() const { return size_; }
...
template <typename T> T *Vector<T>::data() const { return data_; }

template <typename T> T *Vector<T>::begin() const { return &data_[0]; }

template <typename T> T *Vector<T>::end() const { return &data_[size_]; }
...
template <typename Y>
void fillWithPtr(Vector<Y> &v, const void *const ptr,
...
#endif // LUMOS_MATH_LIN_ALG_VECTOR_DYNAMIC_VECTOR_DYNAMIC_H_
